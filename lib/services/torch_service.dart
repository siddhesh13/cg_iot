...
class TorchService {
  ...
  final String tableName = 'command'; // Replace with your Airtable Table Name
    ...
    try {
      ...
      if (response.statusCode == 200) {
        try {
          ...
          final flashlightStatus = data['records'][0]['fields']['command']; // Replace with the actual field name
          ...
          else if (flashlightStatus.toLowerCase() == 'play') {
            await playAudio();
            isTorchOn = false;
            statusMessage = 'ok';
          } else{
  ...
  try {
    ...
    if (await Vibrate.canVibrate) {
      Vibrate.vibrate(); // Simple vibration
    } else {
